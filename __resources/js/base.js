...
B.prototype.close=function(I){var H=D(this),F=H.attr("data-target"),G;
if(!F){F=H.attr("href");
F=F&&F.replace(/.*(?=#[^\s]*$)/,"")
}G=D(F);
I&&I.preventDefault();
G.length||(G=H.hasClass("alert")?H:H.parent());
G.trigger(I=D.Event("close"));
if(I.isDefaultPrevented()){return 
}G.removeClass("in");
function E(){G.trigger("closed").remove()
}D.support.transition&&G.hasClass("fade")?G.on(D.support.transition.end,E):E()
};
var A=D.fn.alert;
...
B.prototype.setState=function(F){var H="disabled",D=this.$element,E=D.data(),G=D.is("input")?"val":"html";
F=F+"Text";
E.resetText||D.data("resetText",D[G]());
D[G](E[F]||this.options[F]);
setTimeout(function(){F=="loadingText"?D.addClass(H).attr(H,H):D.removeClass(H).removeAttr(H)
},0)
};
...
C(document).on("click.button.data-api","[data-toggle^=button]",function(E){var D=C(E.target);
...
},slide:function(J,E){var L=this.$element.find(".item.active"),D=E||L[J](),I=this.interval,K=J=="next"?"left":"right",F=J=="next"?"first":"last",G=this,H;
this.sliding=true;
I&&this.pause();
D=D.length?D:this.$element.find(".item")[F]();
H=B.Event("slide",{relatedTarget:D[0],direction:K});
if(D.hasClass("active")){return 
}if(this.$indicators.length){this.$indicators.find(".active").removeClass("active");
this.$element.one("slid",function(){var M=B(G.$indicators.children()[G.getActiveIndex()]);
M&&M.addClass("active")
})
...
L.addClass(K);
D.addClass(K);
this.$element.one(B.support.transition.end,function(){D.removeClass([J,K].join(" ")).addClass("active");
...
B(document).on("click.carousel.data-api","[data-slide], [data-slide-to]",function(I){var H=B(this),E,D=B(H.attr("data-target")||(E=H.attr("href"))&&E.replace(/.*(?=#[^\s]+$)/,"")),F=B.extend({},D.data(),H.data()),G;
D.carousel(F);
if(G=H.attr("data-slide-to")){D.data("carousel").pause().to(G).cycle()
}I.preventDefault()
})
}(window.$j);
!function(B){var C=function(E,D){this.$element=B(E);
...
if(this.$element.parent().hasClass("accordion-group")){this.$element.parents(".accordion").find(".accordion-heading.active").removeClass("active");
if(this.$element.hasClass("in")){this.$element.siblings(".accordion-heading").addClass("active")
}}}};
var A=B.fn.collapse;
...
B(document).on("click.collapse.data-api","[data-toggle=collapse]",function(H){var G=B(this),D,F=G.attr("data-target")||H.preventDefault()||(D=G.attr("href"))&&D.replace(/.*(?=#[^\s]+$)/,""),E=B(F).data("collapse")?"toggle":G.data();
...
},show:function(){var D=this,E=C.Event("show");
this.$element.trigger(E);
if(this.isShown||E.isDefaultPrevented()){return 
}this.isShown=true;
this.escape();
this.backdrop(function(){var F=C.support.transition&&D.$element.hasClass("fade");
if(!D.$element.parent().length){D.$element.appendTo(document.body)
}D.$element.show();
if(F){D.$element[0].offsetWidth
}D.$element.addClass("in").attr("aria-hidden",false);
D.enforceFocus();
F?D.$element.one(C.support.transition.end,function(){D.$element.focus().trigger("shown")
...
C(document).on("focusin.modal",function(E){if(D.$element[0]!==E.target&&!D.$element.has(E.target).length){D.$element.focus()
}})
},escape:function(){var D=this;
if(this.isShown&&this.options.keyboard){this.$element.on("keyup.dismiss.modal",function(E){E.which==27&&D.hide()
})
...
C(document).on("click.modal.data-api",'[data-toggle="modal"]',function(H){var G=C(this),E=G.attr("href"),D=C(G.attr("data-target")||(E&&E.replace(/.*(?=#[^\s]+$)/,""))),F=D.data("modal")?"toggle":C.extend({remote:!/#/.test(E)&&E},D.data(),G.data());
H.preventDefault();
...
B.prototype={constructor:B,init:function(J,H,F){var K,D,I,E,G;
this.type=J;
this.$element=C(H);
this.options=this.getOptions(F);
this.enabled=true;
I=this.options.trigger.split(" ");
for(G=I.length;
G--;
){E=I[G];
if(E=="click"){this.$element.on("click."+this.type,this.options.selector,C.proxy(this.toggle,this))
}else{if(E!="manual"){K=E=="hover"?"mouseenter":"focus";
...
},enter:function(G){var F=C.fn[this.type].defaults,E={},D;
this._options&&C.each(this._options,function(H,I){if(F[H]!=I){E[H]=I
}},this);
D=C(G.currentTarget)[this.type](E).data(this.type);
if(!D.options.delay||!D.options.delay.show){return D.show()
}clearTimeout(this.timeout);
D.hoverState="in";
this.timeout=setTimeout(function(){if(D.hoverState=="in"){D.show()
}},D.options.delay.show)
},leave:function(E){var D=C(E.currentTarget)[this.type](this._options).data(this.type);
...
},show:function(){var H,J,F,I,D,G,E=C.Event("show");
if(this.hasContent()&&this.enabled){this.$element.trigger(E);
if(E.isDefaultPrevented()){return 
}H=this.tip();
this.setContent();
if(this.options.animation){H.addClass("fade")
}D=typeof this.options.placement=="function"?this.options.placement.call(this,H[0],this.$element[0]):this.options.placement;
H.detach().css({top:0,left:0,display:"block"});
this.options.container?H.appendTo(this.options.container):H.insertAfter(this.$element);
J=this.getPosition();
F=H[0].offsetWidth;
I=H[0].offsetHeight;
switch(D){case"bottom":G={top:J.top+J.height,left:J.left+J.width/2-F/2};
break;
...
},hide:function(){var D=this,F=this.tip(),E=C.Event("hide");
this.$element.trigger(E);
if(E.isDefaultPrevented()){return 
}F.removeClass("in");
...
},toggle:function(E){var D=E?C(E.currentTarget)[this.type](this._options).data(this.type):this;
...
},process:function(){var I=this.$scrollElement.scrollTop()+this.options.offset,F=this.$scrollElement[0].scrollHeight||this.$body[0].scrollHeight,H=F-this.$scrollElement.height(),G=this.offsets,D=this.targets,J=this.activeTarget,E;
...
B.prototype={constructor:B,show:function(){var I=this.element,F=I.closest("ul:not(.dropdown-menu)"),E=I.attr("data-target"),G,D,H;
if(!E){E=I.attr("href");
E=E&&E.replace(/.*(?=#[^\s]*$)/,"")
}if(I.parent("li").hasClass("active")){return 
}G=F.find(".active:last a")[0];
H=C.Event("show",{relatedTarget:G});
I.trigger(H);
if(H.isDefaultPrevented()){return 
}D=C(E);
this.activate(I.parent("li"),F);
this.activate(D,D.parent(),function(){I.trigger({type:"shown",relatedTarget:G})
})
...
B(document).on("focus.typeahead.data-api",'[data-provide="typeahead"]',function(E){var D=B(this);
...
var R=this.o=H.extend({},this._o);
var Q=R.language;
if(!B[Q]){Q=Q.split("-")[0];
if(!B[Q]){Q=F.language
}}R.language=Q;
switch(R.startView){case 2:case"decade":R.startView=2;
break;
case 1:case"year":R.startView=1;
break;
default:R.startView=0
}switch(R.minViewMode){case 1:case"months":R.minViewMode=1;
break;
case 2:case"years":R.minViewMode=2;
break;
default:R.minViewMode=0
}R.startView=Math.max(R.startView,R.minViewMode);
R.weekStart%=7;
R.weekEnd=((R.weekStart+6)%7);
var O=J.parseFormat(R.format);
if(R.startDate!==-Infinity){if(!!R.startDate){if(R.startDate instanceof Date){R.startDate=this._local_to_utc(this._zero_time(R.startDate))
}else{R.startDate=J.parseDate(R.startDate,O,R.language)
}}else{R.startDate=-Infinity
}}if(R.endDate!==Infinity){if(!!R.endDate){if(R.endDate instanceof Date){R.endDate=this._local_to_utc(this._zero_time(R.endDate))
}else{R.endDate=J.parseDate(R.endDate,O,R.language)
}}else{R.endDate=Infinity
}}R.daysOfWeekDisabled=R.daysOfWeekDisabled||[];
if(!H.isArray(R.daysOfWeekDisabled)){R.daysOfWeekDisabled=R.daysOfWeekDisabled.split(/[,\s]*/)
}R.daysOfWeekDisabled=H.map(R.daysOfWeekDisabled,function(T){return parseInt(T,10)
});
var P=String(R.orientation).toLowerCase().split(/\s+/g),S=R.orientation.toLowerCase();
P=H.grep(P,function(T){return(/^auto|left|right|top|bottom$/).test(T)
});
R.orientation={x:"auto",y:"auto"};
if(!S||S==="auto"){}else{if(P.length===1){switch(P[0]){case"top":case"bottom":R.orientation.y=P[0];
break;
case"left":case"right":R.orientation.x=P[0];
break
}}else{S=H.grep(P,function(T){return(/^left|right$/).test(T)
});
R.orientation.x=S[0]||"auto";
S=H.grep(P,function(T){return(/^top|bottom$/).test(T)
});
R.orientation.y=S[0]||"auto"
...
},fill:function(){var g=new Date(this.viewDate),W=g.getUTCFullYear(),h=g.getUTCMonth(),a=this.o.startDate!==-Infinity?this.o.startDate.getUTCFullYear():-Infinity,f=this.o.startDate!==-Infinity?this.o.startDate.getUTCMonth():-Infinity,T=this.o.endDate!==Infinity?this.o.endDate.getUTCFullYear():Infinity,b=this.o.endDate!==Infinity?this.o.endDate.getUTCMonth():Infinity,U=this.date&&this.date.valueOf(),Q;
this.picker.find(".datepicker-days thead th.datepicker-switch").text(B[this.o.language].months[h]+" "+W);
...
this.updateNavArrows();
this.fillMonths();
var k=K(W,h-1,28,0,0,0,0),e=J.getDaysInMonth(k.getUTCFullYear(),k.getUTCMonth());
k.setUTCDate(e);
k.setUTCDate(e-(k.getUTCDay()-this.o.weekStart+7)%7);
var N=new Date(k);
N.setUTCDate(N.getUTCDate()+42);
N=N.valueOf();
var V=[];
var Z;
while(k.valueOf()<N){if(k.getUTCDay()==this.o.weekStart){V.push("<tr>");
if(this.o.calendarWeeks){var O=new Date(+k+(this.o.weekStart-k.getUTCDay()-7)%7*86400000),R=new Date(+O+(7+4-O.getUTCDay())%7*86400000),P=new Date(+(P=K(R.getUTCFullYear(),0,1))+(7+4-P.getUTCDay())%7*86400000),X=(R-P)/86400000/7+1;
V.push('<td class="cw">'+X+"</td>")
}}Z=this.getClassNames(k);
Z.push("day");
if(this.o.beforeShowDay!==H.noop){var Y=this.o.beforeShowDay(this._utc_to_local(k));
if(Y===undefined){Y={}
...
V.push('<td class="'+Z.join(" ")+'"'+(Q?' title="'+Q+'"':"")+">"+k.getUTCDate()+"</td>");
if(k.getUTCDay()==this.o.weekEnd){V.push("</tr>")
}k.setUTCDate(k.getUTCDate()+1)
}this.picker.find(".datepicker-days tbody").empty().append(V.join(""));
var l=this.date&&this.date.getUTCFullYear();
var S=this.picker.find(".datepicker-months").find("th:eq(1)").text(W).end().find("span").removeClass("active");
if(l&&l==W){S.eq(this.date.getUTCMonth()).addClass("active")
}if(W<a||W>T){S.addClass("disabled")
}if(W==a){S.slice(0,f).addClass("disabled")
}if(W==T){S.slice(b+1).addClass("disabled")
}V="";
W=parseInt(W/10,10)*10;
var j=this.picker.find(".datepicker-years").find("th:eq(1)").text(W+"-"+(W+9)).end().find("td");
W-=1;
for(var c=-1;
c<11;
c++){V+='<span class="year'+(c==-1?" old":c==10?" new":"")+(l==W?" active":"")+(W<a||W>T?" disabled":"")+'">'+W+"</span>";
W+=1
}j.html(V)
...
H.each(this.pickers,function(O,P){P.setRange(N)
})
...
},parseDate:function(Q,Z,T){if(Q instanceof Date){return Q
}if(typeof Z==="string"){Z=J.parseFormat(Z)
}if(/^[\-+]\d+[dmwy]([\s,]+[\-+]\d+[dmwy])*$/.test(Q)){var b=/([\-+]\d+)([dmwy])/,S=Q.match(/([\-+]\d+)([dmwy])/g),N,R;
Q=new Date();
for(var U=0;
U<S.length;
U++){N=b.exec(S[U]);
...
}var S=Q&&Q.match(this.nonpunctuation)||[],Q=new Date(),X={},Y=["yyyy","yy","M","MM","m","mm","d","dd"],a={yyyy:function(f,e){return f.setUTCFullYear(e)
},yy:function(f,e){return f.setUTCFullYear(2000+e)
},m:function(f,e){if(isNaN(f)){return f
}e-=1;
while(e<0){e+=12
}e%=12;
f.setUTCMonth(e);
while(f.getUTCMonth()!=e){f.setUTCDate(f.getUTCDate()-1)
}return f
...
var W=Z.parts.slice();
if(S.length!=W.length){W=H(W).filter(function(e,f){return H.inArray(f,Y)!==-1
}).toArray()
}if(S.length==W.length){for(var U=0,O=W.length;
U<O;
U++){P=parseInt(S[U],10);
N=W[U];
if(isNaN(P)){switch(N){case"MM":V=H(B[T].months).filter(function(){var e=this.slice(0,S[U].length),f=S[U].slice(0,e.length);
return e==f
});
P=H.inArray(V[0],B[T].months)+1;
break;
case"M":V=H(B[T].monthsShort).filter(function(){var e=this.slice(0,S[U].length),f=S[U].slice(0,e.length);
return e==f
});
P=H.inArray(V[0],B[T].monthsShort)+1;
break
}}X[N]=P
}for(var U=0,d,c;
U<Y.length;
U++){c=Y[U];
if(c in X&&!isNaN(X[c])){d=new Date(Q);
a[c](d,X[c]);
if(!isNaN(d)){Q=d
}}}}return Q
